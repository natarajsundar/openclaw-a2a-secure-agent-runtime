...
- `src/a2a-client.mjs`
  - Thin JSON-RPC client for sending messages and polling tasks on a remote A2A agent
- `src/agent-card-cache.mjs`
  - Fetches and TTL-caches Agent Cards from remote agent well-known endpoints
- `src/demo.mjs`
  - Runnable example showing one end-to-end delegated task
- `src/mock-remote-agent.mjs`
  - Small in-memory A2A-like server used for local validation and tests
- `src/openclaw-a2a-relay.mjs`
  - Relay logic for discovery, delegation, session-context mapping, and task polling
- `src/session-task-map.mjs`
  - Persists a mapping of OpenClaw sessions to remote A2A tasks and context IDs
- `src/utils.mjs`
  - Shared utility helpers (hashing, UUIDs, text extraction, terminal-state check)
- `test/relay.test.mjs`
  - Minimal test coverage using the Node built-in test runner
- `examples/openclaw-plugin-entry.example.ts`
  - Conceptual OpenClaw plugin shape for future integration (requires `@sinclair/typebox`)
...
The runnable demo and tests use only built-in Node.js modules. The illustrative plugin entry example (`examples/openclaw-plugin-entry.example.ts`) additionally depends on `@sinclair/typebox`; install it separately with `npm install @sinclair/typebox` if you need to run that file.
...
The demo starts the mock remote agent internally; no separate server process is needed.
...
2. Forward the requested `skillId` to the remote agent as metadata (no local skill validation)
...
- correct skill forwarding/propagation (no local validation)
...
- Architecture article: _add your freeCodeCamp URL here_
- Architecture notes: _add architecture notes or links here_
- Demo output examples: _add screenshots or logs here_
- Talk or workshop: _add links here when available_
